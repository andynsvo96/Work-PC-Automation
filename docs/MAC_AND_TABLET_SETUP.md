...
3. Create separate Windows and Mac users in Supabase Authentication. Run the SQL files in `supabase/migrations/` in numeric order in the SQL editor. Existing installations should also apply newly added migrations, including `003_allow_zero_repeat_interval.sql` and `004_retry_failed_tasks.sql`.
